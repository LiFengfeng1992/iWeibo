...
-(void)setCellFrame:(BaseStatusCellFrame *)cellFrame
{
    ...
    StatusModel *s = cellFrame.status;
    ...
    //XLog(@"%@ verifiedType:%d",s.user.screenName, s.user.verifiedType);
    // 判断是不是会员
    ...
    CGSize timeSize = X_TEXTSIZE(_time.text, kTimeFont);
    ...
    CGSize sourceSize = X_TEXTSIZE(_source.text, kSourceFont);
