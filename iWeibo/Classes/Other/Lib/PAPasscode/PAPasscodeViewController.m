    ...
    if (self) {
        ...
        switch (action) {
            case PasscodeActionSet:
                ...
                _enterPrompt = NSLocalizedString(@"输入密码", nil);
                _confirmPrompt = NSLocalizedString(@"再次输入密码", nil);
                break;
                
            case PasscodeActionEnter:
                self.title = NSLocalizedString(@"输入密码", nil);
                _enterPrompt = NSLocalizedString(@"输入密码", nil);
                break;
                
            case PasscodeActionChange:
                self.title = NSLocalizedString(@"修改密码", nil);
                _changePrompt = NSLocalizedString(@"输入原密码", nil);
                _enterPrompt = NSLocalizedString(@"输入新密码", nil);
                _confirmPrompt = NSLocalizedString(@"再次输入新密码", nil);
                break;
        }
        ...
        case PasscodeActionSet:
            if (phase == 0) {
                ...
                if ([text isEqualToString:_passcode]) {
                    ...
                    messageLabel.text = NSLocalizedString(@"密码错误，请重试！", nil);
                }
            }
            break;
...
- (void)showFailedAttempts {
    ...
    failedAttemptsLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%d 次输入错误！", nil), _failedAttempts];
}
...
- (void)showScreenForPhase:(NSInteger)newPhase animated:(BOOL)animated {
    ...
    if (!_simple) {
        ...
        if (finalScreen) {
            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"完成", nil) style:UIBarButtonItemStyleBordered target:self action:@selector(handleCompleteField)];
        } else {
